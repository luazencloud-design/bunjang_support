...
### #10 [Phase 7] Supabase 백엔드 + 태스크 큐 시스템
...
### #11 [Phase 8] PWA 멀티 검색 (번장/당근/쿠팡/메루카리)
**우선순위**: 보통

현재 메루카리만 검색. 한국 매장에서 매입 결정 시 번장·당근·쿠팡 시세도 동시 확인 필요.

- 결과 카드에 4개 사이트 검색 버튼
- 검색어 직접 편집 input (바코드 → 일본어/한글 상품명)
- 마지막 검색어 캐싱

---

### #12 [Phase 9] 베타 모집 + PMF 검증
...
| Vercel PWA 배포 (`vercel.json` 작성됨) | 6 | GitHub 연동 + 자동 빌드 |
| Supabase 프로젝트 셋업 | 7 | 사용자 액션 (Tokyo region) |
| 백엔드 추상화 레이어 (`src/lib/backend/`) | 7 | portable 설계 |
| 매직링크 인증 (PWA·확장 양쪽) | 7 | Supabase Auth |
| 태스크 큐 DB + Storage + Realtime | 7 | #10 |
| 3일 자동 삭제 (pg_cron) | 7 | Supabase 콘솔 |
| PWA 멀티 검색 (번장/당근/쿠팡/메루카리) | 8 | #11 |
| 통계 대시보드 (일별 처리량) | 8 | |
| 베타 5~10명 모집 | 9 | #12 |
| Cloudflare Workers 프록시 (옵션 4 — 하이브리드 API 키) | 10 | PMF 검증 후 |
| 카카오 OAuth | 10 | 한국인 친화적 |
| 포트원 결제 + 월 구독 | 10 | PMF 검증 후 |
| 사업자등록 + 통신판매업 신고 + 약관/개인정보 처리방침 | 10 | 결제 사업화 시 |
